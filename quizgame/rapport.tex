...
\begin{itemize}
  ...
  \item Les clés API (Firebase, OpenRouter) sont stockées dans un fichier .env non versionné et chargées via flutter\_dotenv.
  \item Un fichier .env.example est fourni comme template sans les valeurs sensibles.
\end{itemize}
...
\begin{itemize}
  ...
  \item flutter\_dotenv — gestion sécurisée des clés API via fichier .env
\end{itemize}
...
\begin{itemize}
  ...
  \item flutter\_dotenv permet de sécuriser les clés API en les stockant dans un fichier .env non versionné.
\end{itemize}
...
Flux pseudo-code:
\begin{lstlisting}
0. main(): await dotenv.load(fileName: '.env')
...
3. QuestionGeneratorService.generateQuestions(categories, totalQuestions):
   ...
   c. Recupere API key via: dotenv.env['OPENROUTER_API_KEY']
   d. Appel HTTP POST vers OpenRouter API (xiaomi/mimo-v2-flash:free)
   e. Parse JSON, nettoie les blocs markdown si presents
   f. Question.fromJson() pour chaque question
   g. Si erreur: _generateFallbackQuestions()
...
lib/
|-- main.dart                    # Point d'entree + deep links + dotenv init
|-- firebase_options.dart        # Configuration Firebase (via dotenv)
|-- screens/                     # Ecrans (UI)
...
|   |-- question_generator_service.dart  # Generation IA (via dotenv)
...
# Fichiers de configuration securite (racine projet)
...
Cette section présente les captures d'écran des différentes interfaces de Quizzly.

\subsection{Menu Principal et Authentification}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.7\textwidth]{assets/main_menu.png}
  \caption{Écran du Menu Principal avec animation d'étoiles}
  \label{fig:main-menu}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.7\textwidth]{assets/auth_screen.png}
  \caption{Écran d'Authentification (Email/Google Sign-In)}
  \label{fig:auth}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.7\textwidth]{assets/avatar_picker.png}
  \caption{Sélection d'Avatar avec les 28 animaux disponibles}
  \label{fig:avatar}
\end{figure}

\newpage

\subsection{Lobby et Configuration}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.9\textwidth]{assets/lobby.png}
  \caption{Écran Lobby avec liste des joueurs et QR Code}
  \label{fig:lobby}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.9\textwidth]{assets/game_settings.png}
  \caption{Paramètres de jeu (temps, rounds, catégories)}
  \label{fig:settings}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.9\textwidth]{assets/categories.png}
  \caption{Sélection des catégories de questions}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.9\textwidth]{assets/countdown.png}
  \caption{Compte à rebours avant chaque question}
  \label{fig:countdown}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.9\textwidth]{assets/question.png}
  \caption{Affichage d'une question avec les 4 options}
  \label{fig:question}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.9\textwidth]{assets/answer_feedback.png}
  \caption{Feedback après réponse (correct/incorrect)}
  \label{fig:feedback}
\end{figure}

\newpage

\subsection{Résultats et Statistiques}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.9\textwidth]{assets/leaderboard.png}
  \caption{Podium animé avec les 3 premiers joueurs}
  \label{fig:leaderboard}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.9\textwidth]{assets/confetti.png}
  \caption{Célébration du gagnant avec confettis}
  \label{fig:confetti}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.7\textwidth]{assets/player_stats.png}
  \caption{Statistiques du joueur (parties, victoires, score moyen)}
  \label{fig:stats}
\end{figure}
...
Cette section documente les mesures de sécurité implémentées dans Quizzly.

\subsection{Gestion Sécurisée des Clés API}

Le projet utilise \texttt{flutter\_dotenv} pour stocker toutes les clés API sensibles dans un fichier \texttt{.env} non versionné.

\subsubsection*{Fichier .env.example (template sans valeurs)}
\begin{lstlisting}[language=bash]
# Firebase API Keys
...
FIREBASE_IOS_BUNDLE_ID=com.example.yourapp

# OpenRouter API Key
OPENROUTER_API_KEY=your_openrouter_api_key_here
\end{lstlisting}

\subsubsection*{Chargement des Variables d'Environnement}
Dans \texttt{main.dart}:
\begin{lstlisting}[language=java]
...
\subsubsection*{Utilisation dans firebase\_options.dart}
\begin{lstlisting}[language=java]
import 'package:flutter_dotenv/flutter_dotenv.dart';
...
\subsubsection*{Utilisation dans question\_generator\_service.dart}
\begin{lstlisting}[language=java]
import 'package:flutter_dotenv/flutter_dotenv.dart';
...
Le fichier \texttt{.gitignore} exclut les fichiers sensibles du contrôle de version:
\begin{lstlisting}[language=bash]
# Environment files
.env
.env.local
...
\subsection{Configuration pubspec.yaml}
Le fichier \texttt{.env} est déclaré comme asset pour être inclus dans le bundle:
...
\begin{itemize}
  \item Les clés API ne sont jamais commitées dans le dépôt Git.
  \item Chaque développeur peut avoir son propre fichier \texttt{.env} local.
  \item Le fichier \texttt{.env.example} sert de documentation pour les clés requises.
  \item Facilite le déploiement avec différentes configurations (dev, staging, production).
  \item Configuration centralisée pour Firebase (Web, Android, iOS) et OpenRouter API.
\end{itemize}
...
\begin{itemize}
  ...
  \item http: \^{}1.2.0
  \item flutter\_dotenv: \^{}5.2.1 — gestion sécurisée des variables d'environnement
\end{itemize}
